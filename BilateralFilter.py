...
if __name__ == '__main__':
    ...
    original_image = imread(input_name)

    if original_image.shape[2] == 3:                    # RBG image
        print("RGB")
        color_filtered_image = color_bilateral_filter(original_image)
        plt.imshow(color_filtered_image/255)
        plt.show()
        imwrite(output_name, color_filtered_image.astype(np.uint8))

    else:                                               #black and white image
        print("black and white")
        BW_filtered_image = bilateral_filter(original_image)
        imwrite(output_name, BW_filtered_image)
